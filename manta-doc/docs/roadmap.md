...
## 현재 초점 (2026-08): Fyne dogfood 실험

> **Phase 1~7 (CLI-first + Wails) 구현은 일시 정지.**  
> 상세 합의: [experiment-fyne-jira-local.md](experiment-fyne-jira-local.md)
...
| 구현 홈 | 형제 폴더 **`manta-fyne/`** (독립 go.mod / git) |
| 동결 | `manta-repo` (`cmd/manta` / Wails) 확장 중지 |
| 종료 | MVP+실사용 후, 형식 고통이 반복될 때 계약 freeze (작성자 판단) |

장기 spine(아래 Phase 1~7)은 폐기한 것이 아니라 **실험 종료 후 재평가**한다.
Manifesto의 파일 원본·local-first·AI-usable 방향은 유지하고, **전달 순서만** 실험한다.
...
| 이전 (폐기) | Go rewrite (동결 중) | 현재 실험 |
|---|---|---|
| TypeScript / npm workspaces | Go module | Go module (동일) |
| Electron | Wails (`desktop/`) | **Fyne** (`manta-fyne/`) |
...
실험 결정: [experiment-fyne-jira-local.md](experiment-fyne-jira-local.md)
...
> ⏸ 실험 중 일시 정지. 재개 시 파일 계약은 실험 결과를 반영해 수정할 수 있다.
...
> ⏸ 실험 중 일시 정지. 실험 구간 SQLite는 레포 로컬 인덱스만.
...
> ⏸ 실험 중 일시 정지. 1차 dogfood UI는 Fyne ([experiment-fyne-jira-local.md](experiment-fyne-jira-local.md)).
> 실험 후 Wails 재개·폐기·병행은 미정.
...
> 실험 중에는 **Jira REST 단방향·특정 키·최소 필드 import만** 한다.
