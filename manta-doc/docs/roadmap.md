...
Markdown 파일이 source of truth이고, SQLite는 빠른 조회와 제품 경험을 위한 재생성 가능한 인덱스다.
...
- `manta index rebuild` — Markdown 파일을 스캔해 인덱스를 다시 만든다
- `manta index check` — 인덱스가 파일 상태와 맞는지 확인한다

### 원칙

- SQLite에만 존재하는 사용자 데이터는 두지 않는다.
- SQLite는 삭제해도 Markdown에서 다시 만들 수 있어야 한다.
- 인덱스가 깨져도 작업 파일은 안전해야 한다.
- search, context, GUI는 인덱스를 사용해 빨라질 수 있지만, 원본은 항상 파일이다.
- SQLite 구현은 `@manta/core`의 무거운 런타임 의존성을 만들지 않도록 별도 모듈이나 adapter 경계에서 다룬다.

### 완료 기준

- Markdown task 폴더를 스캔해 SQLite 인덱스를 만들 수 있다.
- 파일 변경 후 인덱스를 재생성할 수 있다.
- 인덱스가 없어도 사용자가 원본 task 파일을 잃지 않는다.
...
- GUI는 SQLite를 표시/검색용 인덱스로 사용할 수 있다.
